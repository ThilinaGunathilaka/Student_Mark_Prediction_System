...
Enter 'y' for yes or 'q' to quit and view result: """))
  ...
  if (outcome == 'y'):
      ...
      with open(r"C:\Users\thili\OneDrive\Desktop\History.txt","w")as history:   # w - write
        for list_antiant1 in list_1:
            history.write('%s' % list_antiant1 + "\n")
        for list_antiant2 in list_2:
            history.write('%s' % list_antiant2 + "\n")
        for list_antiant3 in list_3:
            history.write('%s' % list_antiant3 + "\n")
        for list_antiant4 in list_4:
            history.write('%s' % list_antiant4 + "\n")
      break
  else:
        print("This string not valid! try again")
        continue
# reference
#https://www.codegrepper.com/code-examples/python/save+a+list+to+text+file+python
